...
# Processing for each file------------------------------------------------------------------
        ...
        # Extract data between the second last and last 'SUMMARY'
        relevant_data = data[second_last_summary_index:last_summary_index]
    
        # Split by newlines to get individual lines
        lines = relevant_data.split('\n')
        
        # Initialize a list to store the RF2 data
        rf2_relevant_data = []
        rf1_relevant_data = []

        #----------------------------------------------------------
        # Track when to start collecting data
        start_collecting = False
        # Find and store RF2 values
        for line in lines:
            if 'RF2' in line:
        ...
        # Remove the first two and the last lines
        rf2_relevant_data = rf2_relevant_data[2:-1]
        
        # Track when to start collecting data
        start_collecting = False
        # Find and store RF1 values
        for line in lines:
            if 'RF1' in line:
                start_collecting = True
            elif start_collecting and line.strip().startswith('MAXIMUM'):
        ...
        # Remove the first two and the last lines
        rf1_relevant_data = rf1_relevant_data[2:-1]
        #---------------------------------------------------------
        
        # Initialize the lists for collecting RF2 and RF1 data
        ...
        # Check if the lists are empty before calculating sum
